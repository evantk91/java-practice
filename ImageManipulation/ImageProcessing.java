...
public class ImageProcessing {
  public static void main(String[] args) {
    ...
    twoDToImage(negativeImg, "./neg_kitten.jpg");

    int[][] stretchedHorizImg = stretchHorizontally(imageData);
    twoDToImage(stretchedHorizImg, "./stretch_horiz_kitten.jpg");

    int[][] shrinkedVertImg = shrinkVertically(imageData);
    twoDToImage(shrinkedVertImg, "./shrink_vert_kitten.jpg");

    int[][] invertedImg = invertImage(imageData);
    twoDToImage(invertedImg, "./inverted_kitten.jpg");

    int[][] filteredImg = colorFilter(imageData, -75, 30, -30);
    twoDToImage(filteredImg, "./filtered_kitten.jpg");
    ...
    int[][] randRectImg = generateRectangles(randomImg, 30);
    twoDToImage(randRectImg, "./randRectangle.jpg");
  }
  ...
  public static int[][] negativeColor(int[][] imageTwoD) {
    ...
    int[][] processedImg = new int[imageTwoD.length][imageTwoD[0].length];
    for(int i = 0; i < imageTwoD.length; i++) {
      for(int j = 0; j < imageTwoD[i].length; j++) {
        ...
        rgba[1] = 255 - rgba[1];
        rgba[2] = 255 - rgba[2];
        processedImg[i][j] = getColorIntValFromRGBA(rgba);
      }
    } 
	return processedImg;
  }
  ...
  public static int[][] stretchHorizontally(int[][] imageTwoD) {
	// TODO: Fill in the code for this method
    int[][] processedImg = new int[imageTwoD.length][imageTwoD[0].length * 2];
    int it = 0;
    for(int i = 0; i < imageTwoD.length; i++) {
      for(int j = 0; j < imageTwoD[i].length; j++) {
        it = j * 2;
        processedImg[i][it] = imageTwoD[i][j];
        processedImg[i][it + 1] = imageTwoD[i][j];
      }
    }
	return processedImg;
  }
  ...
  public static int[][] shrinkVertically(int[][] imageTwoD) {
	// TODO: Fill in the code for this method
    int[][] processedImg = new int[imageTwoD.length / 2][imageTwoD[0].length];
    for(int i = 0; i < imageTwoD[0].length; i++) {
      for(int j = 0; j < imageTwoD.length - 1; j += 2) {
        processedImg[j / 2][i] = imageTwoD[j][i];
      }
    }
	return processedImg;
  }
  ...
  public static int[][] invertImage(int[][] imageTwoD) {
    ...
    for(int i = 0; i < imageTwoD.length; i++) {
      for(int j = 0; j < imageTwoD[0].length; j++) {
        processedImg[i][j] = imageTwoD[(imageTwoD.length - 1) - i][(imageTwoD[0].length - 1) - j]; 
      }  
    }
	return processedImg;
  }
  ...
  public static int[][] generateRectangles(int[][] canvas, int numRectangles) {
    ...
    for(int i = 0; i < numRectangles; i++) {
      ...
      paintRectangle(canvas, randWidth, randHeight, randRowPos, randColPos, randColor);
    }
	return canvas;
  }
